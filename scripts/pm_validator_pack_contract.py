...
BINDING_REQUIRED_FIELDS = (
    "id", "binding_type", "match", "symbol_role", "authoritative_semantics",
    "peer_renderers", "shared_contract_refs", "downstream_consumers",
    "exception_state_refs", "required_wiring", "forbidden", "required_validation",
    "verification_mode", "verification_method", "semantic_group", "conflict_policy",
    "oracle_ref",
)


def _rr(i, s, d): return R(i, s, d)
...
def _normalize_single_ascii_line(raw):
    text = _dar(raw)
    if text is None: return None, "non_ascii"
    if "\r" in text: return None, "must_use_lf"
    value = text[:-1] if text.endswith("\n") else text
    if "\n" in value: return None, "must_have_exactly_one_line"
    return (None, "must_be_non_empty") if value == "" else (value, None)


def _su(bindings, field): return sorted({i for b in bindings for i in b.get(field, [])})


def _bm(bindings, key, value): return {binding[key]: binding[value] for binding in bindings}


def _read_instructions_zip(path):
    out = [_rr("INSTRUCTIONS_EXTENSION", "PASS" if path.suffix == ".zip" else "FAIL", str(path))]
    if path.suffix != ".zip": return out, None, None, None
    ...
    out.append(_rr("INSTRUCTIONS_HANDOFF", "PASS" if hok else "FAIL",
    ...
    hv, he = (
        (None, "missing_hash_pack")
        if hash_raw is None else _normalize_single_ascii_line(hash_raw)
    )
    ...
    if pack_raw is None or hv is None:
        ...
        out.append(_rr("PACK_HASH_INTEGRITY", "PASS" if actual == hv else "FAIL",
...
def _load_jsonl_bytes(raw):
    ...
    if text is None: raise VE("spec_not_utf8")
    out = []
    for idx, line in enumerate(text.splitlines(), start=1):
        if not line.strip(): continue
        try: obj = json.loads(line)
        except json.JSONDecodeError as exc: raise VE(f"spec_jsonl_invalid_line:{idx}:{exc.msg}")
        if not isinstance(obj, dict): raise VE(f"spec_jsonl_non_object:{idx}")
        out.append(obj)
    return out
...
def _collect_binding_meta_and_bindings(objects):
    ...
    for obj in objects:
        ...
        if kind == "binding_meta":
            if meta is not None: raise VE("binding_meta_duplicate")
            meta = obj; continue
        if kind == "oracle":
            oid = str(obj.get("id", "")).strip()
            if not oid: raise VE("oracle_missing_id")
            if oid in oracles: raise VE(f"oracle_duplicate:{oid}")
            oracles[oid] = obj; continue
        if kind != "obligation_binding": continue
        bid = str(obj.get("id", "<missing-id>"))
        missing = [field for field in BINDING_REQUIRED_FIELDS if field not in obj]
        if missing: raise VE(f"binding_missing_fields:{bid}:{','.join(missing)}")
        if obj["binding_type"] not in SBT:
            raise VE(f"binding_type_unsupported:{bid}:{obj['binding_type']}")
        for field in (
            "verification_mode", "verification_method", "semantic_group", "conflict_policy"
        ):
            if not str(obj.get(field, "")).strip(): raise VE(f"binding_empty_field:{bid}:{field}")
        bindings.append(obj)
    if meta is None: raise VE("binding_meta_missing")
...
def _ensure_binding_consistency(active_bindings, oracles):
    if not active_bindings: raise VE("binding_active_missing")
    symbols, semantics, roles = {}, {}, {}
    for binding in active_bindings:
        bid = str(binding.get("id", "<missing-id>"))
        ref = str(binding.get("oracle_ref", "")).strip()
        if not ref: raise VE(f"binding_missing_oracle_ref:{bid}")
        if ref not in oracles: raise VE(f"binding_unknown_oracle_ref:{bid}:{ref}")
    ...
    for ids in symbols.values():
        if len(ids) > 1: raise VE(f"binding_ambiguous_symbol:{','.join(sorted(ids))}")
    for ids in semantics.values():
        if len(ids) > 1: raise VE(f"binding_duplicate_semantics:{','.join(sorted(ids))}")
    for role, vals in roles.items():
        if len(vals) > 1: raise VE(f"binding_conflicting_obligations:{role}")


def _build_pack_from_spec_bytes(spec_raw, mode, target_scope):
    objs = _load_jsonl_bytes(spec_raw)
    if not objs or objs[0].get("type") != "meta": raise VE("spec_meta_missing")
    ...
    pack = {
        ...
        "aggregate_scope_metadata": {"binding_count": len(active), "mode": mode,
                                     "target_scope": target_scope},
    }
...
def _authority_spec_bytes(args):
    ...
    if spec in overlay: return overlay[spec], None
    if args.workspace_snapshot and spec in set(args.supplemental_file):
        snap = _iz(Path(args.workspace_snapshot))
        if spec in snap: return snap[spec], None
...
def _scope_mapping_rule(decision_paths, pack):
    ...
    if not decision_paths: return _rr("PACK_SCOPE_MAPPING", "FAIL", "no_patch_paths")
    if scope == "authority_scope":
        bad = [p for p in decision_paths if not (p.startswith("docs/") or p.startswith("scripts/"))]
        return _rr("PACK_SCOPE_MAPPING", "PASS" if not bad else "FAIL",
...
def _forbidden_bypass_rule(patch_member_names, pack, active_bindings):
    ...
    if act != exp: return _rr("PACK_FORBIDDEN_BYPASS", "FAIL", f"expected={exp}:actual={act}")
    ...
    if leaked:
        return _rr("PACK_FORBIDDEN_BYPASS", "FAIL",
                   f"patch_contains_instruction_artifacts:{leaked}")
...
def _recompute_pack_rule(args, pack, pack_raw):
    ...
    if err is not None or spec_raw is None:
        return _rr("PACK_RECOMPUTE", "UNVERIFIED_ENVIRONMENT",
                   err or "missing_authority_spec"), None
    ...
    try: rebuilt, _rebuilt_hash, active = _build_pack_from_spec_bytes(spec_raw, mode, scope)
    except VE as exc: return _rr("PACK_RECOMPUTE", "FAIL", str(exc)), None
    if pack_raw is None: return _rr("PACK_RECOMPUTE", "FAIL", "missing_pack_bytes"), active
    status = "PASS" if rebuilt == pack_raw else "FAIL"
    return _rr("PACK_RECOMPUTE", status,
               "recompute_match" if status == "PASS" else "recompute_mismatch"), active
...
def _pack_rule_verdicts(pack, active_bindings, support_rules):
    ...
    for binding in bindings:
        ...
        if not mode:
            out.append(_rr(f"PACK_RULE:{bid}", "FAIL", "missing_verification_mode")); continue
        if mode != "machine":
            out.append(_rr(f"PACK_RULE:{bid}", "MANUAL_REVIEW_REQUIRED", f"mode={mode}")); continue
        ...
        if fail:
            ...
            out.append(_rr(f"PACK_RULE:{bid}", status, first.rule_id)); continue
        detail = (
            f"mode={binding.get('verification_mode')} "
            f"method={binding.get('verification_method')}"
        )
...
def _verdict_coverage_rule(pack, verdicts):
    ...
    return _rr("PACK_VERDICT_COVERAGE", "PASS" if act == exp else "FAIL",
...
def _pack_rules(args, instructions_path, decision_paths, patch_member_names):
    ...
    if pack is None: return out, None
    ...
    out.append(_pack_union_rule("PACK_REQUIRED_WIRING", pack, "required_wiring", active,
                                "required_wiring"))
    out.append(_forbidden_bypass_rule(patch_member_names, pack, active))
    out.append(_pack_union_rule("PACK_DOWNSTREAM_COVERAGE", pack, "downstream_consumers",
                                active, "downstream_consumers"))
    out.append(_pack_union_rule("PACK_REQUIRED_VALIDATION", pack, "required_validation",
                                active, "required_validation"))
    out.append(_scope_mapping_rule(decision_paths, pack))
    watch = {"PACK_HASH_INTEGRITY", "PACK_RECOMPUTE", "PACK_REQUIRED_WIRING",
