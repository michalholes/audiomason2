...
def _read_log_tail_text(log_path: Path, *, tail_suffix: str = ".tail.txt") -> str | None:
    tail_path = log_path.with_name(log_path.name + tail_suffix)
...
def iter_runs(patches_root: Path, log_filename_regex: str) -> list[RunEntry]:
    ...
    for log_path in sorted(matched_paths):
        ...
        try:
            ...
            continue
        tail = _read_log_tail_text(log_path)
        if tail is not None:
