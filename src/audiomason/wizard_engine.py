...
class WizardEngine:
    ...
    def _execute_input_step(self, step: dict, context: ProcessingContext) -> StepResult:
        ...
        if required and not value:
            ...
            if fallback:
                ...
                return StepResult(
                    success=False,
                    error=f"Required field: {step.get('id')}"
                )
    ...
    def _execute_plugin_call(self, step: dict, context: ProcessingContext) -> StepResult:
        ...
        try:
            if hasattr(plugin, method):
                ...
                return StepResult(
                    success=False,
                    error=f"Method not found: {plugin_name}.{method}"
                )
        except Exception as e:
            return StepResult(
                success=False,
                error=f"Plugin call failed: {str(e)}"
            )
    ...
    def run_wizard(self, wizard_def: dict, context: ProcessingContext | None = None) -> ProcessingContext:
        """Run complete wizard.
    ...
    def run_wizard_from_file(self, path: Path, context: ProcessingContext | None = None) -> ProcessingContext:
