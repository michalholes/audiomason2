...
""",
    ...
    def _assert_cfg() -> None:
        ...
        cfg = _make_cfg(ctx.repo_root, cfg_path.relative_to(ctx.repo_root), "verbose", None, None, None)
        joined = " ".join(cfg.runner_cmd)
        assert "--verbosity=verbose" in joined, f"FAIL: expected --verbosity=verbose in runner_cmd, got: {joined}"
...
TEST = {
    "name": "test_021_runner_verbose_override_cli",
    "run": run,
}
