...
class FileIOSync:
    ...
    def detect_sources(self, inbox_path: Optional[Path] = None) -> list[SourceInfo]:
        ...
        for item in sorted(search_dir.iterdir()):
            ...
            if item.is_dir():
                ...
                self._log_debug(f"Directory {item.name}: {len(subdirs_with_audio)} subdirs with audio, {len(files_in_root)} audio files in root")
    ...
    def export_to_output(self, context: ProcessingContext) -> ProcessingContext:
        ...
        for src_file in context.converted_files:
            ...
            self._log_debug(f"Moving: {src_file.name} -> {author_clean}/{title_clean}/{dest_file.name}")
