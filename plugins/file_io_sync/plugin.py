...
class FileIOSync:
    ...
    def detect_sources(self, inbox_path: Path | None = None) -> list[SourceInfo]:
        """Detect all sources in inbox directory.

        Supports:
        - autor/kniha/ structure (detects as separate books)
        - autor/*.mp3 structure (single book, needs title)
        - standalone directories with audio
        - archives
        ...
        for item in sorted(search_dir.iterdir()):
            ...
            # Check if directory
            if item.is_dir():
                # Check if this is autor/kniha structure
                subdirs_with_audio = []
                files_in_root = []

                for subitem in item.iterdir():
                    if subitem.is_dir() and self._has_audio_files(subitem):
                        subdirs_with_audio.append(subitem)
                    elif subitem.is_file() and subitem.suffix.lower() in self.AUDIO_EXTS:
                ...
                if subdirs_with_audio:
                    for kniha_dir in subdirs_with_audio:
                        has_cover = self._has_cover_files(kniha_dir)
                        sources.append(
                            SourceInfo(
                                path=kniha_dir,
                                type="directory",
                                name=f"{item.name}/{kniha_dir.name}",
                                has_audio=True,
                                has_cover=has_cover,
                            )
                        )
                        self._log_debug(f"Found autor/kniha: {item.name}/{kniha_dir.name}")

                # If has audio files in root -> single book (needs title)
                if files_in_root:
                    ...
                    self._log_debug(f"Found directory with audio: {item.name}")

            # Check if archive
            elif item.is_file() and item.suffix.lower() in self.ARCHIVE_EXTS:
                sources.append(
                    SourceInfo(path=item, type="archive", name=item.name, has_audio=True)
                )
            ...
            # Check if standalone audio file
                ...
                sources.append(
                    SourceInfo(path=item, type="audio_file", name=item.name, has_audio=True)
                )
    ...
    def _copy_directory(self, src: Path, dst: Path) -> None:
        ...
        for item in src.rglob("*"):
            if item.is_file():
                ...
                files_copied += 1
                self._log_debug(f"Copied: {rel_path}")

        self._log_debug(f"Total files copied: {files_copied}")
    ...
    def export_to_output(self, context: ProcessingContext) -> ProcessingContext:
        ...
        Creates structure: output/autor/kniha/*.mp3
        ...
        # Create structure: autor/kniha/
        output_path = self.output_dir / author_clean / title_clean

        self._log_info(f"Exporting to: {output_path}")
        self._log_debug(f"Structure: {author_clean}/{title_clean}/")
        ...
        for src_file in context.converted_files:
            ...
            self._log_debug(
                f"Moving: {src_file.name} -> {author_clean}/{title_clean}/{dest_file.name}"
            )
