(function () {
  ...
  async function _getPathPrefixes() {
    ...
    _prefixesCache = (Array.isArray(items) ? items : []).map((x) => String(x || ""));
  ...
  function renderTransitions(opts) {
    ...
    btnAdd.addEventListener("click", function () {
      ...
      if (t === "true") whenVal = true;
        ...
        else if (t === "not_exists") whenVal = { op: "not_exists", path: fullPath };
        else if (t === "eq" || t === "neq") {
    ...
    outgoingWithIndex
      ...
      .forEach(function (e, idx) {
        ...
        row.appendChild(text("div", "flowTransCellPri", String(edge.priority || 0)));
        row.appendChild(text("div", "flowTransTo", String(edge.to_step_id || "")));
        row.appendChild(text("div", "flowTransMeta", _summarizeWhen(edge.when)));
