(function () {
  ...
  function renderUnsupported(ftype, fld) {
    const meta = fld && typeof fld === "object" ? JSON.stringify(fld, null, 2) : "";
    const msg = "Unsupported field type: " + String(ftype || "") + "\n\n" + meta;
    setText("stepError", msg);
  }

  function readAnswer(state, name) {
    const ans = state && state.answers && typeof state.answers === "object" ? state.answers : null;
    if (ans && name in ans) return ans[name];
    const inp = state && state.inputs && typeof state.inputs === "object" ? state.inputs : null;
  ...
  function renderField(fld, stepId) {
    ...
    if (ftype === "number") {
      ...
      const c = fld && typeof fld.constraints === "object" ? fld.constraints : null;
    ...
    if (ftype === "multi_select_indexed") {
      ...
      box.appendChild(el("div", { class: "hint", text: "Selection is sent as " + name + "_ids" }));
      return box;
    }
  ...
  function collectPayload(stepId) {
    ...
    const nodes = ui.step ? Array.from(ui.step.querySelectorAll("input,select,textarea")) : [];
    const grouped = {};
    ...
    nodes.forEach((n) => {
      ...
      if (n.tagName.toLowerCase() === "input" && n.getAttribute("type") === "number") {
  ...
  refresh().catch((e) => setText("status", String(e && e.message ? e.message : e)));
})();
