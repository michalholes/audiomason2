...
class BasicWizardSync:
    ...
    def _log_debug(self, msg: str) -> None:
        ...
        if self.verbosity >= 3 and self.workflow.should_show_messages(self.verbosity, 'config_values'):
            print(f"[DEBUG] {msg}")

    def _log_verbose(self, msg: str) -> None:
        """Log verbose message (verbosity >= 2)."""
        if self.verbosity >= 2 and self.workflow.should_show_messages(self.verbosity, 'plugin_calls'):
            print(f"[VERBOSE] {msg}")
    ...
    def _log_workflow(self, msg: str) -> None:
        ...
        if self.verbosity >= 1 and self.workflow.should_show_messages(self.verbosity, 'workflow_steps'):
            print(msg)
    ...
    def run_workflow(self, source_path: Optional[Path] = None) -> ProcessingContext:
        ...
        sources = file_io.detect_sources(source_path)
        ...
        if self.workflow.should_show_messages(self.verbosity, 'prompts'):
            ...
            answer = self._prompt("Choose source number, or 'a' for all", "1")
            ...
            if answer == 'a':
                ...
                try:
                    ...
                    if 1 <= num <= len(sources):
                        selected_sources = [sources[num - 1]]
                    else:
                        raise WizardError("Invalid source number")
                except ValueError:
