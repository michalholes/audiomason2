...
def main():
    ...
    # Processing options
    parser.add_argument('--clean-inbox', choices=['yes', 'no', 'ask'], help='Clean inbox after import')
    parser.add_argument('--clean-stage', choices=['yes', 'no', 'ask'], help='Clean stage after import')
    parser.add_argument('--publish', choices=['yes', 'no', 'ask'], help='Publish to output')
    parser.add_argument('--wipe-id3', choices=['yes', 'no', 'ask'], help='Wipe ID3 tags before tagging')
    ...
    # Resolve all config
    resolved_config = {}
    for key in ['inbox_dir', 'stage_dir', 'output_dir', 'verbosity', 
                'clean_inbox', 'clean_stage', 'publish', 'wipe_id3',
                'bitrate', 'loudnorm', 'split_chapters']:
        try:
