...
def test_api_jobs_enqueue_subset_creates_derived_zip_and_provenance(tmp_path: Path) -> None:
    s = _mk_self(tmp_path)
    zpath = s.patches_root / "issue_506_demo.zip"
    entries = [
        "patches/per_file/scripts__patchhub__app_api_jobs.py.patch",
...
def test_api_jobs_enqueue_rejects_raw_command_with_selected_patch_entries(tmp_path: Path) -> None:
    s = _mk_self(tmp_path)
    zpath = s.patches_root / "issue_506_demo.zip"
