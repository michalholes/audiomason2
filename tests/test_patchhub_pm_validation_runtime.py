...
def test_zip_manifest_includes_pm_validation_and_prefers_success_archive(tmp_path: Path) -> None:
    s = _mk_self(tmp_path)
...
def test_build_pm_validation_uses_repair_overlay_only_when_available(tmp_path: Path) -> None:
    s = _mk_self(tmp_path)
    relpath = "scripts/sample.py"
    before = "def value():\n    return 2\n"
    after = "def value():\n    return 3\n"
...
def test_build_pm_validation_repair_escalates_with_exact_supplemental_files(tmp_path: Path) -> None:
    s = _mk_self(tmp_path)
