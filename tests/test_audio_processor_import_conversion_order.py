...
def test_plan_import_conversion_targets_mp3_for_supported_import_formats(
    ...
    assert [Path(plan[0]["output"]).suffix for plan in planned] == [".mp3", ".mp3", ".mp3"]


def test_plan_import_conversion_sorts_split_chapter_order_for_m4b(tmp_path: Path) -> None:
...
def test_build_split_command_keeps_seek_timing_and_loudnorm_order(tmp_path: Path) -> None:
    plugin = AudioProcessorPlugin({"split_chapters": True, "loudnorm": True})
...
def test_build_conversion_command_keeps_loudnorm_and_codec_order(tmp_path: Path) -> None:
