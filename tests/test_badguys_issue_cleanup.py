...
def test_cleanup_issue_artifacts_removes_current_artifacts_each_time(tmp_path: Path) -> None:
...
def test_cleanup_issue_artifacts_preserves_other_issue_artifacts(tmp_path: Path) -> None:
    ctx = _make_ctx(tmp_path)
    patches_dir = ctx.cfg.patches_dir
