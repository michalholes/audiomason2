...
def test_resolve_config_path_uses_embedded_layout_default(tmp_path: Path) -> None:
    _, resolve_config_path, _, _, _ = _amp_symbols()
    runner_root = tmp_path / "runner"
    import_root = runner_root / "scripts"
    package_dir = import_root / "am_patch"
    package_dir.mkdir(parents=True)
    cfg_path = package_dir / "am_patch.toml"
    cfg_path.write_text('verbosity = "normal"\n', encoding="utf-8")

    assert resolve_config_path(None, runner_root, import_root) == cfg_path
...
def test_legacy_repo_root_must_obey_registry(tmp_path: Path) -> None:
    ...
    denied.mkdir()
    policy = policy_cls(
        target_repo_roots=[str(allowed)],
        repo_root=str(denied),
    )
