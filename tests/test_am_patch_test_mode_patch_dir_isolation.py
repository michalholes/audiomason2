...
def test_test_mode_json_out_uses_persistent_patch_root_json_dir(tmp_path: Path) -> None:
    policy_cls, build_paths_and_logger, runtime_mod = _import_am_patch()
    ...
    try:
        ...
        ctx = build_paths_and_logger(cli, policy, cfg, "test")
        expected_json = ctx.patch_root / policy.patch_layout_json_dir / "am_patch_issue_999.jsonl"

        assert ctx.json_path == expected_json
        assert ctx.json_path.parent == ctx.patch_root / policy.patch_layout_json_dir
        assert ctx.json_path.parent != ctx.paths.json_dir
    finally:
        if ctx is not None:
            if ctx.ipc is not None:
                ctx.ipc.stop()
