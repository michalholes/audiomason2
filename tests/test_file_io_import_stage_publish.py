...
def test_stage_publish_and_cleanup_surfaces_keep_root_relative_model(tmp_path: Path) -> None:
...
def test_stage_import_path_unpacks_archive_into_deterministic_work_tree(tmp_path: Path) -> None:
    plugin = _plugin(tmp_path)
...
def test_publish_import_path_uses_deterministic_fallback_when_target_exists(tmp_path: Path) -> None:
    plugin = _plugin(tmp_path)
    source_dir = tmp_path / "inbox" / "Author" / "Book"
    source_dir.mkdir(parents=True)
