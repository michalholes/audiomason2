...
def test_failure_zip_target_is_derived_from_selected_effective_root(tmp_path: Path) -> None:
    scripts_dir = Path(__file__).parent.parent / "scripts"
    sys.path.insert(0, str(scripts_dir))
...
def test_nonworkspace_target_repo_name_uses_target_selection_contract(tmp_path: Path) -> None:
    from types import SimpleNamespace
...
def test_workspace_invalid_patch_target_fails_before_root_binding(tmp_path: Path) -> None:
    from types import SimpleNamespace
